...
export function CommunityEvents() {
  ...
  const getEventTypeColor = (title: string) => {
    ...
    if (title.toLowerCase().includes("conservation")) return "bg-emerald-100 text-emerald-800"
    if (title.toLowerCase().includes("heritage")) return "bg-amber-100 text-amber-800"
  ...
  const getLocationEmoji = (location: string) => {
    ...
    if (location.includes("Spiti")) return "🌌"
    if (location.includes("Kinnaur")) return "🍑"
    if (location.includes("Chamba")) return "🏛️"
    if (location.includes("Mandi")) return "🕌"
    if (location.includes("Solan")) return "🍄"
    if (location.includes("Sirmaur")) return "🦆"
    if (location.includes("Hamirpur")) return "🏰"
    if (location.includes("Una")) return "🙏"
    if (location.includes("Bilaspur")) return "🌊"
    return "🏞️"
  }
  ...
  const eventsByDistrict = mockCommunityEvents.reduce(
    (acc, event) => {
      const district = event.district || "Other"
      if (!acc[district]) acc[district] = []
      acc[district].push(event)
      return acc
    },
    {} as Record<string, typeof mockCommunityEvents>,
  )
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600">Join local eco-warriors across all 12 districts of DevBhoomi and earn points!</p>
          <div className="mt-4 grid grid-cols-2 gap-4">
            <div className="bg-green-50 p-3 rounded-lg border border-green-200">
              <div className="text-lg font-bold text-green-800">🏔️ Nature Conservation</div>
              <div className="text-sm text-green-600">Earn 100-200 points per event</div>
            </div>
            <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
              <div className="text-lg font-bold text-blue-800">🌱 Sustainable Living</div>
              <div className="text-sm text-blue-600">Learn & earn eco-points</div>
            </div>
          </div>
        </div>

        {/* Events by District */}
        {Object.entries(eventsByDistrict).map(([district, events]) => (
          <div key={district} className="mb-8">
            <h2 className="text-xl font-semibold text-gray-800 mb-4 flex items-center gap-2">
              <span className="text-2xl">{getLocationEmoji(events[0]?.location || "")}</span>
        ...
        <Card className="mt-8 bg-green-50 border-green-200">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-green-700 mb-4">
              Help build the community by organizing cleanup drives, workshops, or cultural events in your district.
            </p>
